            ...
            if(strstr(json_encode($usersList, JSON_UNESCAPED_UNICODE), $marzbanText) && !empty($marzbanText)){
                ...
                foreach($usersList as $key => $config){
                    if(strstr(json_encode($config->links, JSON_UNESCAPED_UNICODE), $marzbanText)){
                        ...
                        $expiryTime = $config->expire != 0?jdate("Y-m-d ",$config->expire):"نامحدود";
                    ...
                    if(!isset($list[0]->clientStats)){
                        foreach($list as $keys=>$packageInfo){
                            if(strpos($packageInfo->settings, $config_link)!=false){
                                ...
                                $state = $packageInfo->enable == true?"فعال 🟢":"منقضی 🔴";
                                $totalUsed = sumerize2($packageInfo->up + $packageInfo->down);
                                $total = $packageInfo->total!=0?sumerize2($packageInfo->total):"نامحدود";
                                $expiryTime = $packageInfo->expiryTime != 0?jdate("Y-m-d ",substr($packageInfo->expiryTime,0,-3)):"نامحدود";
                        ...
                        if(!isset($clientsSettings[$settingKey]['email'])){
                            ...
                            if($clientState[$emailKey]->total != 0 || $clientState[$emailKey]->up != 0  ||  $clientState[$emailKey]->down != 0 || $clientState[$emailKey]->expiryTime != 0){
                                ...
                                $expiryTime = $expTime != 0?jdate("Y-m-d ",substr($expTime,0,-3)):"نامحدود";
                                $expiryDay = $expTime != 0?
                                    floor(
                                ...
                                $state = $clientState[$emailKey]->enable == true?"فعال 🟢":"منقضی 🔴";
                                $remark = $email;
                            }
                            elseif($list[$keys]->total != 0 || $list[$keys]->up != 0  ||  $list[$keys]->down != 0 || $list[$keys]->expiryTime != 0){
                                ...
                                $expiryTime = $list[$keys]->expiryTime != 0?jdate("Y-m-d ",substr($list[$keys]->expiryTime,0,-3)):"نامحدود";
                                ...
                                $state = $list[$keys]->enable == true?"فعال 🟢":"منقضی 🔴";
    ...
    <?php if ($type=="configInfo"){
        ...
        <div class="container" style="">
            <form id="contact" class="contactw">
                <br>
                <p style="font-size:22px;font-weight: bold;color:#1d3557;font-family:iransans !important;"><?php echo $remark;?></p>
                <p style="font-size:18px;font-weight: bold;color:#1d3557;margin-top:15px;"> وضعیت اکانت :  <?php echo $state;?> </p>
                <div class="container">
                    <p class="tarikh" style="font-size:18px;font-weight: bold;color:#1d3557;margin-top:15px;">
                       تاریخ انقضا  
                       <br><span><?php echo $expiryTime;?></span>
                    </p>
                </div>
                ...
                <br>
                <hr>
                <p style="font-size:18px;font-weight: bold;color:#1d3557;margin-top:15px;"> مسیحلو 💜 </p>
